...
MM=function(x,N,alpha,beta){
  ...
  repeat{
    ...
    if(abs(theta$theta-theta0)<0.001) return(list(v$v_tilde,theta$theta))
...
est3=function(x,g,an,beta,i,n){ # estimate each omega
  ...
  if(i==n) return(list(i0=n,out=(n-i+1+an)/(2.5*30*g*(1-Fx(x[i]))+beta)/n))
  outs[1:(n-i)]=sapply(i:(n-1),function(j){return((j-i+1+an)/(2.5*30*g*(Fx(x[j+1])-Fx(x[i]))))}) # j changes
  outs[n-i+1]=(n-i+1+an)/(2.5*30*g*(1-Fx(x[i]))+beta*n)
  outn=min(outs)
